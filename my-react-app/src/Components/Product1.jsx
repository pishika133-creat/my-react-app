...
function handleBuy(item){
    ...
    alert("buy this product :",item.name )
}

console.log("recipie",recipie.recipes)

  return (
    <>
            <h2>Top Recipie </h2>
    <div style ={{display:"flex",alignItems:"center",justifyContent:"space-evenly",gap:"10px",flexWrap:"wrap"}}>

        {
            recipie.recipes.map((item ,i )=>(
                <div key={i}
                style={{width:"250",border:"1px solid gray",padding:"10px",borderRadius:"10px"}}>
                    <img width={200} height={100}  src={item.image} alt="" />
                    <p>{item.name}</p>
                    <p>{item.rating}</p>
                    <p>{item.mealType[0]}</p>
                    <p>{item.tags[0]}</p>
                    <p>{item.tags[1]}</p>
                    <p>{item.caloriesPerServing}kal</p>
                    <button onClick={()=>handleBuy(item)}>Buy Now : {item.userId}</button>
                </div>
            ))
